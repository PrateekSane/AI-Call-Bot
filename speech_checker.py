    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Is the following text likely to be human speech? Respond with only 'yes' or 'no': '{transcript}'"}
            ]
        )
        
        is_human = response.choices[0].message['content'].strip().lower() == 'yes'
        return {"is_human": is_human, "transcript": transcript}
